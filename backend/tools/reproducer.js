...
async function writeReadmeAtomic(filePath, frontmatter, content, useTemp = true) {
  const text = matter.stringify(content || '', frontmatter || {});
  if (!useTemp) {
    await fs.writeFile(filePath, text, 'utf8');
    return;
  }
  const tmp = `${filePath}.tmp.${Date.now()}.${Math.floor(Math.random() * 100000)}`;
...
async function pollApi(project, task, expectedStatus, timeoutMs = 3000, pollInterval = 50) {
  const url = `${SERVER}/api/tasks/${encodeURIComponent(project)}/${encodeURIComponent(task)}`;
  ...
  while (Date.now() < end) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        const actual = (json.frontmatter && json.frontmatter.status) || json.status || null;
...
async function main() {
  ...
  console.log('Reproducer starting', { SERVER, DATA_ROOT, project, task, iterations, delayMs, filePath });
  ...
  async function callStatusUpdateApi(project, task, status) {
    const url = `${SERVER}/api/tasks/${encodeURIComponent(project)}/${encodeURIComponent(task)}/status`;
    try {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    console.log(
      `[${i}] status=${status} wroteAt=${afterWrite - start}ms fileStatus=${fileStatus} apiOk=${poll.ok} apiActual=${poll.last ? JSON.stringify(poll.last) : 'match'} attempts=${poll.attempts}`
    );
    ...
    if (!ok) {
      ...
      const logPath = path.join(__dirname, '..', '..', 'logs', `reproducer-${Date.now()}.json`);
