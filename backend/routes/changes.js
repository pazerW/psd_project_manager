    ...
    const id = setTimeout(async () => {
      ...
      try {
        if (type !== 'readme-removed' && (await fs.pathExists(filePath))) {
          ...
          status = parsed.data && parsed.data.status !== undefined ? parsed.data.status : null;
        ...
        console.error('[changes] Failed to read README for status:', filePath, err);
      }

      const payload = JSON.stringify({ type, path: rel, status, frontmatter, mtime, lastUpdated });
